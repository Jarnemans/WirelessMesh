...
/* ---------------------------------------------------------------------
 * Defining all the models and their neccecities
 * ---------------------------------------------------------------------
...
static void bt_ready(int err)
{
    ...
    /* Execute a shell command after initialization */
    const struct shell *shell = shell_backend_uart_get_ptr();
    
    if (shell) {
        shell_execute_cmd(shell, "Hello there!");
    } else {
...
static int board_init(void)
{
    ...
    /* Button init */
    const struct device *btn_dev = DEVICE_DT_GET(DT_GPIO_CTLR(BUTTON_NODE, gpios));
