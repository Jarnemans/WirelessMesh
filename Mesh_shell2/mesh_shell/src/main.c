 ...
 * -------------------------------------------------------------
 * We'll pick an example Company ID. The official range is
 * assigned by the Bluetooth SIG, but for demonstration, we
 * can pick something like 0x1234 or 0x05F1, etc.
 */
#define MY_COMPANY_ID      0x1234
#define MY_CUSTOM_MODEL_ID 0x0001

/* 
 * The custom opcode must be defined using BT_MESH_MODEL_OP_3 
 * if it's vendor-specific. 
 * Example: opcode = 0x05, plus our vendor company ID.
 */
...
/* 
 * Vendor model receive callback. We keep the same logic 
 * from your 'receive_custom_message' function, just updated 
 * for a vendor model. 
 */
...
/* If you want to broadcast updated OnOff state to all nodes. */
static int send_onoff_message(bool state)
{
    static uint8_t tid;
    struct bt_mesh_msg_ctx ctx = {
